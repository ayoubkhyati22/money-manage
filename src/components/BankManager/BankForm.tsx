...
export function BankForm({
  ...
  return (
    <div className="bg-white/90 dark:bg-dark-800/90 backdrop-blur-sm rounded-2xl shadow-lg border border-mint-200/50 dark:border-dark-600/50 overflow-hidden">
      <div className="bg-gradient-to-r from-emerald-50 to-mint-50 dark:from-emerald-900/30 dark:to-mint-900/30 p-3 sm:p-6 border-b border-emerald-200 dark:border-dark-600">
        <div className="flex items-center space-x-2 sm:space-x-3">
          <div className="flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 bg-emerald-500 rounded-xl">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-emerald-700 dark:text-emerald-300 hidden sm:block">
          ...
          <div className="flex items-center space-x-2 sm:space-x-3 pt-2 sm:pt-4">
            ...
            >
              {loading ? (
                ...
                <span className="text-sm">{editingBank ? 'Update' : 'Add'}</span>
              )}
            </button>
            <button
              type="button"
              onClick={onCancel}
              className="text-gray-600 dark:text-dark-300 hover:text-gray-900 dark:hover:text-dark-100 px-4 py-2 sm:px-6 sm:py-3 rounded-xl transition-colors font-medium text-sm"
            >
              Cancel
            </button>
          </div>
